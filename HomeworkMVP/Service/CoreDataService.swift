...
class CoreDataStack {
    ...
    private lazy var managedObjectModel: NSManagedObjectModel = {
        let url = Bundle.main.url(forResource: "HomeworkMVP", withExtension: "momd")!
        return NSManagedObjectModel(contentsOf: url)!
    }()
    // swiftlint:enable force_unwrapping
    
    private lazy var persistentStoreDescription: NSPersistentStoreDescription = {
        let databaseFileName = "HomeworkMVP.sqlite"
    ...
    func saveMainContext(_ closure: (() -> Void)? = nil) {
        self.mainContext.perform {
            do {
                ...
                print("save Main Context error: \(nserror.userInfo)\n")
    ...
    func saveBackgroundContext(_ closure: (() -> Void)? = nil) {
        self.backgroundContext.perform {
            do {
                ...
                let nserror = exception as NSError
                print("save Background Context error: \(nserror.userInfo)\n")
                fatalError("Unresolved error \(nserror), \(nserror.userInfo)")
            }
        }
    }
